...
import { ArrowLeft, Plus, Trash2, Edit3, Check, X, Link2 } from 'lucide-react';
...
const BUILTIN_KEYS = ['expenseTypes', 'counterparties', 'employees', 'contractors', 'paymentForms', 'shiftTypes'];
...
export default function Departments({ user }) {
  ...
  const saveItem = () => {
    ...
    if (refType === 'employees' && editPin) {
      const dup = refs.employees?.find(e => e.pin === editPin && e.id !== editId);
      if (dup) return alert('PIN-код уже используется сотрудником ' + dup.name);
    }
    const updated = { ...refs };
    if (editId) {
    ...
    <div>
      ...
      <div className="form-group">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <select className="form-select" value={refType} onChange={e => { setRefType(e.target.value); cancelEdit(); }} style={{ flex: 1 }}>
      ...
      {/* Add item button */}
      <button className="btn btn-primary" onClick={startCreate} style={{ marginBottom: 16, width: '100%', padding: '14px 20px' }}>
